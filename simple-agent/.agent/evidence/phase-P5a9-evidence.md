# Phase P5a.9 证据 — 第 3 步全文待审 + 第 4 步两项前置

- 生成时间：2026-09-15T14:16:02+08:00  分支：simple-agent @ ae61c5d

## 第 3 步：SYSTEM_PROMPT 草稿全文（供作者圈定子集）
...
选型理由：GT 动作类型序列（read/write/compute/think/transfer 多重集）+ <3 小族确定性合并。
比实体归一化 md5 更简单：无需 NLP/实体识别，且天然不暴露 instruction 明文，只存 md5。

```
$ python scripts/tau_ingest.py
[tau_ingest] raw=165 dropped=6 written=159 (train=135, dev=24)
[tau_ingest] families=13 max_share=25.9% min_size=3 sizes=[3, 3, 3, 4, 4, 4, 7, 9, 13, 13, 16, 21, 35]
[tau_ingest] report -> /root/simple-agent/.agent/reports/tau-ingest.md
# REQ 级族分布（train 135）
families: 13 | max share: 25.9% | min size: 3
# 红线：只存 md5，无 instruction 明文
0
```

## 第 4 步.2：cache instrumentation

```
# DeepSeek 返回字段探测
has prompt_cache_hit_tokens : True  0
has prompt_cache_miss_tokens: True  1214
# 接入后 llm_call span 实测（REQ-001）
854|640|214
695|512|183
613|384|229
```
...
- [ ] 第 3 步：作者圈定 P1-P5 子集，POLICY 段（版本化）落地
- [x] 签名归一化验收通过（13 族 / max 25.9% / min 3）
- [x] cache instrumentation 落地（DeepSeek 返回 hit/miss，已接入 llm_call span）
- [ ] epoch-4：前置全绿后解锁
